...
do
    master_token=`ssh -o StrictHostKeyChecking=no master "export KUBECONFIG='/local/repository/kube/admin.conf' && sudo kubeadm token list | grep authentication | cut -d' ' -f 1"`;
    sleep 1;
done
